...
### Fixed
- Fixed `gemmi.DensityCalculatorX` simulating `NaN` structure factors for PDBs containing invalid or extreme anisotropic B-factor (`ANISOU`) records by adding an `ignore_anisou=True` flag to `simulate_diffraction`.
